...
const HOST_API_PORT = process.env.GHOSTBOX_API_PORT || "8008";
const HOST_API_BASE = `http://host.docker.internal:${HOST_API_PORT}`;
const GHOST_NAME = process.env.GHOSTBOX_GHOST_NAME || "";
...
type MailboxAction = "check" | "inbox" | "read" | "send" | "reply";
type ScheduleAction = "create" | "list" | "delete";
type MemoryTarget = keyof typeof MEMORY_LIMITS;
...
const getHostHeaders = (body?: unknown): Record<string, string> => ({
  ...(body === undefined ? {} : { "Content-Type": "application/json" }),
  ...(GHOST_API_KEY ? { Authorization: `Bearer ${GHOST_API_KEY}` } : {})
});

const readJsonResponse = async (response: Response, context: string): Promise<unknown> => {
...
const getMemoryPath = (target: MemoryTarget): string => {
  return target === "memory" ? MEMORY_PATH : USER_PATH;
};

const readMemoryFile = async (target: MemoryTarget): Promise<string> => {
  try {
...
const splitMemoryEntries = (content: string): string[] => {
  if (!content.trim()) {
    return [];
  }
...
const joinMemoryEntries = (entries: string[]): string => {
  return entries.join(MEMORY_SEPARATOR);
};

const assertWithinMemoryLimit = (target: MemoryTarget, nextContent: string): void => {
...
const writeMemoryFile = async (target: MemoryTarget, content: string): Promise<void> => {
  assertWithinMemoryLimit(target, content);
  await writeFile(getMemoryPath(target), content.length > 0 ? `${content}\n` : "");
};

const formatMemoryShow = async (target?: MemoryTarget): Promise<string> => {
  const targets: MemoryTarget[] = target ? [target] : ["memory", "user"];
  const sections = await Promise.all(
    targets.map(async (currentTarget) => {
...
const appendMemoryEntry = async (target: MemoryTarget, content: string): Promise<string> => {
...
const replaceMemoryEntry = async (target: MemoryTarget, search: string, content: string): Promise<string> => {
...
const removeMemoryEntry = async (target: MemoryTarget, search: string): Promise<string> => {
  const trimmedSearch = search.trim();
  if (!trimmedSearch) {
    throw new Error("search is required.");
  }
...
server.registerTool(
  ...
  async ({ action, to, subject, body, messageId, priority }) => {
    const mailboxAction: MailboxAction = action;
    ...
    switch (mailboxAction) {
      ...
      case "read": {
        ...
        const markRead = await hostRequest(
          "POST",
          `/api/mail/${encodeURIComponent(messageId)}/read`,
          undefined,
          "Mailbox read"
        );
      ...
      case "reply": {
        ...
        }) as { from?: unknown; subject?: unknown; threadId?: unknown; id?: unknown } | undefined;
...
server.registerTool(
  ...
  async ({ action, cron, prompt, id, once, timezone }) => {
    const scheduleAction: ScheduleAction = action;
    ...
    switch (scheduleAction) {
      ...
      case "delete": {
        ...
        const payload = await hostRequest(
          "DELETE",
          `${scheduleBasePath}/${encodeURIComponent(id)}`,
          undefined,
          "Schedule delete"
        );
        return { content: [{ type: "text", text: formatJson(payload) }] };
