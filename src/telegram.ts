...
const formatGhostTable = (ghosts: Record<string, { model: string; status: string; portBase: number }>): string => {
  const header = ["NAME".padEnd(12), "MODEL".padEnd(34), "STATUS".padEnd(10), "PORTS"].join("  ");

  const rows = Object.entries(ghosts).map(([name, ghost]) => {
    const ports = `${ghost.portBase}-${ghost.portBase + 9}`;
    return [name.padEnd(12), ghost.model.padEnd(34), ghost.status.padEnd(10), ports].join("  ");
  });
...
const setActiveGhostForChat = async (chatId: string, ghostName: string): Promise<void> => {
...
export const startBot = async (token: string): Promise<void> => {
  ...
  bot.command("spawn", async (ctx) => {
    ...
    try {
      ...
      await spawnGhost(ghostName, state.config.defaultProvider, model);
