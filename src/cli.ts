...
import type { GhostApiKey, GhostboxState, GhostState, AuthProvider } from './types';
...
const promptForInitAdapters = async (): Promise<AuthProvider[]> => {
  ...
  const openaiAuthed = status.providers.openai.authenticated;
...
const verifyGitHubPushAccess = async (
  token: string,
  owner: string,
  repo: string,
): Promise<boolean> => {
...
const performGitHubSetup = async (): Promise<{
  ...
  try {
    ...
    if (createResponse.status === 422) {
      ...
      console.log(`  ${CHECKMARK} Created ${chalk.bold(repo.full_name ?? `${username}/${repoName}`)} ${chalk.dim('(private)')}`);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
const performTelegramSetup = async (): Promise<string> => {
  ...
  const token = await getRequiredInput(
    '  Telegram bot token: ',
...
const init = async (forceReset = false): Promise<void> => {
  ...
  const hasAdapters = authBefore.providers.anthropic.authenticated || authBefore.providers.openai.authenticated;
  ...
  const alreadyOpenai = authBefore.providers.openai.authenticated;
  ...
  const hasOpenai = authAfterLogin.providers.openai.authenticated;
