...
function buildHtmlPage(input: { title: string; message: string; details?: string }): string {
  const details = input.details ? `<p style="color:#666;font-size:14px;">${input.details}</p>` : "";
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
...
async function exchangeAnthropicCode(code: string, verifier: string, state: string): Promise<OAuthTokenRecord> {
...
export async function loginProvider(providerInput: AuthProvider | OAuthProviderDefinition): Promise<OAuthTokenRecord> {
